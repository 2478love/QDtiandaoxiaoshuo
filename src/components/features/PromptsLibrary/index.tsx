        ...
        <div className="flex flex-wrap gap-3">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium border ${activeCategory === cat.id ? 'bg-[#2C5F2D] text-white border-[#2C5F2D]' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}
            >
              {cat.label}
            </button>
          ))}
          ...
          filteredPrompts.map(prompt => (
            <div key={prompt.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-5 flex flex-col gap-3">
              ...
              <button
                ...
                className="mt-2 text-xs text-[#2C5F2D] dark:text-[#97BC62] underline"
              >
                复制提示词
              </button>
            </div>
